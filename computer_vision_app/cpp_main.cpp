...
void ThreadClass::startThread(std::map<std::string,std::string> AllSettingsMap, std::map<std::string,std::string> OneSettingsMap)
{
//    try {
//        QUrl url = QString::fromStdString("http://via.placeholder.com/1000.jpg");
...
//            cv::imshow("render : " + UtilitesClass::GetValueFromMap(OneSettingsMap, "alias_cam"), image_source);
//            cv::waitKey(1);
//        }
...
//                cv::imwrite(UtilitesClass::GetValueFromMap(OneSettingsMap, "alias_cam") + ".jpg", image_source);
//                cv::namedWindow("render : " + UtilitesClass::GetValueFromMap(OneSettingsMap, "alias_cam"), cv::WINDOW_AUTOSIZE);
...
//                           std::stoi(UtilitesClass::GetValueFromMap(AllSettingsMap, "render_size")) / 80.0, cv::INTER_LINEAR);
//                cv::namedWindow("mask : " + UtilitesClass::GetValueFromMap(OneSettingsMap, "alias_cam"), cv::WINDOW_AUTOSIZE);
//                cv::imshow("mask : " + UtilitesClass::GetValueFromMap(OneSettingsMap, "alias_cam"), mask);
//                cv::waitKey(1);
                reply->deleteLater();
                manager->deleteLater();
                cv::destroyWindow("render : " + UtilitesClass::GetValueFromMap(OneSettingsMap, "alias_cam"));
            }
...
void MainClass::on_START_btn_clicked()
{
    ...
    for (auto& OneSettingsMap : AllSettingsVector)
    {
        ...
        QFuture<void> future = QtConcurrent::run(&ThreadClass::startThread, AllSettingsMap, OneSettingsMap);

//        QFuture<void> future = QtConcurrent::run([=]() {
////            ThreadClass obj;
////            obj.startThread(OneSettingsMap);
//            ThreadClass::startThread(OneSettingsMap);
//        });
    }
    if (Playing){
        QCoreApplication::processEvents();
